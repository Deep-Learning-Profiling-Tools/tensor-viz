...
describe('runtime validation', () => {
    ...
    it('preserves marker coords and rejects out-of-bounds markers', () => {
        const manifest = createBundleManifest({
            tensors: [{ name: 'holes', dtype: 'float32', shape: [2, 2], markerCoords: [[1, 1]] }],
        });
        expect(validateBundleManifest(manifest).tensors[0]?.markerCoords).toEqual([[1, 1]]);
        expect(() => validateBundleManifest({
            ...manifest,
            tensors: [{ ...manifest.tensors[0]!, markerCoords: [[2, 0]] }],
