...
describe('parseTensorView', () => {
    ...
    it('fills missing serialized editor fields from defaults', () => {
        const initial = parseTensorView([2, 3, 4], '');
        expect(initial.ok).toBe(true);
        if (!initial.ok) return;
        const { viewTensorInput: _viewTensorInput, ...staleEditor } = initial.spec.editor;
        const result = parseTensorView([2, 3, 4], `tv2:${encodeURIComponent(JSON.stringify(staleEditor))}`);
        expect(result.ok).toBe(true);
        if (!result.ok) return;
        expect(result.spec.editor.viewTensorInput).toBe('[A=2, B=3, C=4]');
        expect(result.spec.viewShape).toEqual([2, 3, 4]);
    ...
    it('maps grouped view slices with non-zero indices back to tensor coordinates', () => {
        const result = parseTensorView([32, 4, 4], serializeTensorViewEditor({
            version: 2,
            viewTensorInput: '[T=32, WR=16]',
            baseDims: [],
            permutedDimIds: [],
            flattenSeparators: [],
            singletons: [],
            slicedTokenKeys: ['group:dim-1'],
            sliceValues: { 'group:dim-1': 9 },
        }), undefined, ['T', 'W', 'R']);
        expect(result.ok).toBe(true);
        if (!result.ok) return;
        expect(mapViewCoordToTensorCoord([7], result.spec)).toEqual([7, 2, 1]);
        expect(visibleTensorCoords(result.spec).slice(0, 3)).toEqual([[0, 2, 1], [1, 2, 1], [2, 2, 1]]);
    });
    ...
    it('enumerates visible tensor coordinates for composed view-permute-view slices', () => {
        const result = parseTensorView([2, 3, 4], serializeTensorViewEditor({
            version: 2,
            viewTensorInput: '[A=2, B=3, C=4]',
            finalViewInput: '[X=4, Y=3, Z=2]',
            baseDims: [],
            permutedDimIds: ['dim-2', 'dim-1', 'dim-0'],
            flattenSeparators: [true, true],
            singletons: [],
            slicedTokenKeys: ['view:Y'],
            sliceValues: { 'view:Y': 1 },
        }));
        expect(result.ok).toBe(true);
        if (!result.ok) return;
        expect(visibleTensorCoords(result.spec)).toEqual([
            [0, 1, 0],
            [1, 1, 0],
            [0, 1, 1],
            [1, 1, 1],
